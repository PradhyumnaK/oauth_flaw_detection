...
def train_gbc(X, y, name: str):
    ...
    print(
        ...
        f"{classification_report(y_test, y_pred, target_names=[INV_LABEL_MAP[i] for i in sorted(INV_LABEL_MAP)])}"
    )
...
def main():
    ...
    #Train gradient bossting on rule features only
    train_gbc(X_rules, y, name="gbc_rules")

    #Train gradient boosting on full feature vector
    train_gbc(X_full, y, name="gbc_full")
